...
int main(int argc, char** argv){
	...
	if( argc < 1+2 ){
		cerr << "Usage: LowrankALBCD [data] [lambda] (eta) (max_iter) (cache_size) (exemplar_cost)" << endl;
		exit(0);
	}
	...
	vector<SparseVec*> data;
	vector<int> labels;
	LIBSVMFileParser::parseSVM(dataFile, D, data, labels);
	D += 1;//adding bias, feature started at index 1
	N = data.size();
	cerr << "D=" << D << ", N=" << N << ", lambda=" << lambda << endl;
